...
public class MyPoiOverlay {
    ...
    public int getLastMarkerPosition() {
        return getPoiIndex(mlastMarker);
    }
    ...
    public MyPoiOverlay(Context context,AMap amap ,List<MapPoiBean> pois) {
    ...
    public void addAllToMap() {
        for (int i = 0; i < mPois.size(); i++) {
            ...
            PoiItem item = parseMapPoiBeanToPoiItem(mPois.get(i));
            marker.setObject(item);
            mPoiMarks.add(marker);
        }
    }

    private PoiItem parseMapPoiBeanToPoiItem(MapPoiBean mapPoiBean) {
        LatLonPoint latLonPoint = new LatLonPoint(mapPoiBean.getLatitude(),mapPoiBean.getLongitude());
        PoiItem poiItem = new PoiItem(mapPoiBean.getPoiId(),latLonPoint,mapPoiBean.getName(),mapPoiBean.getAddress());
    ...
    private MarkerOptions getMarkerOptions(int index) {
        return new MarkerOptions()
                .position(
                        new LatLng(mPois.get(index).getLatitude(), mPois.get(index)
                                .getLongitude()))
    ...
    protected String getTitle(int index) {
        return mPois.get(index).getName();
    }
