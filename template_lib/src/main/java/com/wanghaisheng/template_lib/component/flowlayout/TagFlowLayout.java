    ...
    private void dataChanged() {
        ...
        for (int i = 0, n = adapter.getCount(); i < n; i++) {
            ...
            if (!(view instanceof TagView) && mCheckable) {
                ...
                if (adapter.select(i)) {
                    mPreSelectedItem.add(i);
                    LogUtils.d("tagflowlayout  select...s");
                    adapter.onUnSelect(this, view, i);
                }
            }
...
//        LogUtils.d("viewclickableset  ");
        if (!mAdapter.enabled(position)) return;

        view.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
//                LogUtils.d("view onclick....");
            ...
            if (!TextUtils.isEmpty(mSelectPos)) {
                ...
                if (mSelectedItem.size() > 0) {
                    for (int i : mPreSelectedItem) {
                        if (!mCheckable)
                            mAdapter.onSelect(this, getChildAt(i), i);
                        else
                ...
                for (int index : mSelectedItem) {
                    if (mCheckable) {
                        ...
                        mAdapter.onUnSelect(this, getChildAt(index), index);
                    }
                }
