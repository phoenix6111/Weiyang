...
public class MySimpleDraweeView extends SimpleDraweeView {
    ...
    public void init() {
        if (!isInEditMode()) {
            this.mContextRef = new WeakReference<>(BaseApplication.context().getBaseContext());
            mPlaceholderDrawable = new ColorDrawable(ContextCompat.getColor(mContextRef.get(), R.color.primary_content_bg));
        }
    ...
    public MySimpleDraweeView setGifChartIdentify(final boolean isShowGifIdentify) {
        if (isShowGifIdentify) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                mGifChartOverlay = mContextRef.get().getDrawable(R.drawable.identify_gif);
            } else {
                mGifChartOverlay = mContextRef.get().getResources().getDrawable(R.drawable.identify_gif);
            }
        } else {
    ...
    public MySimpleDraweeView setNumberChartIdentify(int number) {
        if (number > 1) {
            ...
            if (NinePatch.isNinePatchChunk(chunk)) {
                mNumberChartOverlay = new MyNinePatchDrawable(getResources(), bitmap, chunk
                        , NinePatchChunk.deserialize(chunk).mPaddings, null, number + "");
            } else {
...
//            LogUtils.d("set uriPahTag      "+uriPathTag+"      getTag      "+MySimpleDraweeView.this.getTag(R.id.uriPath));
        }
    }
    ...
    private void setHierarchy() {
        ...
        final GenericDraweeHierarchy hierarchy = new GenericDraweeHierarchyBuilder(mContextRef.get().getResources())
    ...
    private void setRoundHierarchy() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            defaultIcon = mContextRef.get().getDrawable(R.drawable.round_default_icon);
        } else {
            defaultIcon = mContextRef.get().getResources().getDrawable(R.drawable.round_default_icon);
        }
    ...
    /**
     * 判定Tag和Url是否相等，相等代表图片已经加载过，不需要重新加载
     */
    private boolean noRepeatLoadImage(String imgUrl) {
//        LogUtils.d(" check repeat  imgUrl   "+imgUrl+"   tag   "+MySimpleDraweeView.this.getTag(R.id.uriPath));
//        return !TextUtils.isEmpty(this.getTag(R.id.uriPath) + "");
//        return !(TextUtils.isEmpty(imgUrl) || TextUtils.isEmpty(this.getTag(R.id.uriPath) + "")) && !(this.getTag(R.id.uriPath) + "").equals(imgUrl);

//        return (!TextUtils.isEmpty(imgUrl))&&((null==this.getTag(R.id.uriPath))||!(this.getTag(R.id.uriPath) + "").equals(imgUrl));
        return !TextUtils.isEmpty(imgUrl);
        /*if (TextUtils.isEmpty(imgUrl)) {
            return false;
        }

        if((this.getTag(R.id.uriPath)==null)|| TextUtils.isEmpty(this.getTag(R.id.uriPath) + "")) {
            return true;
        }
    ...
    private MySimpleDraweeView setControllerByType(final Uri uri, final String type) {
        ...
        if (noRepeatLoadImage(uri.toString())) {
    ...
    public MySimpleDraweeView setLongChartIdentify(final boolean isLongChartIdentify) {
        if (isLongChartIdentify) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                mLongChartOverlay = mContextRef.get().getDrawable(R.drawable.identify_long);
            } else {
                mLongChartOverlay = mContextRef.get().getResources().getDrawable(R.drawable.identify_long);
            }
        } else {
